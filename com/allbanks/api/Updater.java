    ...
    private boolean versionCheck(String title) {
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(" v").length == 2) {
                ...
                if(remoteVersion.equalsIgnoreCase("1.6.5")){
                	this.result = Updater.UpdateResult.NO_UPDATE;
                	return false;
                }
            } else {
                    ...
                    if (Updater.this.versionCheck(Updater.this.versionName)) {
                    	...
                    	for (Player player : Bukkit.getServer().getOnlinePlayers()) {
                			if(player.isOp()||player.hasPermission("allbanks.info.updateavaible")){
                				...
                				player.sendMessage(ChatColor.GOLD+"[AllBanks] "+ChatColor.AQUA+"New version found: "+Updater.this.versionName);
